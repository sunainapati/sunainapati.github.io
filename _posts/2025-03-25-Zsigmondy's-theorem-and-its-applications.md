...
# Introduction 
This is a more refined and self-contained version of \href{https://angyansheng.github.io/blog/an-elementary-proof-of-zsigmondys-theorem}{Yan Sheng's blog post}, \href{https://math.stackexchange.com/questions/660585/elementary-proof-of-zsigmondys-theorem/662196#662196}{Bart Michels's paper} and \href{https://artofproblemsolving.com/community/c1803h1043308_zsigmondys_theorem}{this AoPS post}. Zsigmondy's Theorem states that if \( a > b \geq 1 \) are coprime integers and \( n \geq 2 \), then there exists a prime divisor of \( a^n - b^n \) that does not divide \( a^k - b^k \) for all \( 1 \leq k < n \), except in the following cases:
\begin{itemize}
...
\begin{itemize}
 \item We show it for $v_p(n)=0.$ That is show $v_p(x^n-y^n)=v_p(x-y)$, for $v_p(n)=0$.
...
\item We show it for $v_p(n)=1.$ That is show $v_p(x^n-y^n)=v_p(x-y)+1$
...
but $p \nmid n',y$. So done!
\item Let's assume it's true for $v_p=0,1,\dots, j-1.$ Now, we will show it's true for $v_p(n)=j.$
...
\section{ Orders and Cyclotomic Polynomials}
...
<div class="proof">
We begin with cases. \newline
\textbf{Case 1 }: Note that if $k=n$ then $p\nmid a^n-b^n$. Hence
$$\begin{aligned}
...
\textbf{Case 1 }: If $p\geq3$, since $k<n$ and $p|\Phi_n(a,b)$. We get that $v_p(\Phi_n(a,b))=1$ by the lemma 7.1. So $n$ is of the forms $p^{\beta}k$.We also know that $k|n$ and $k|p-1\implies k<p$. So note that $p$ is the largest prime factor of $n$. Suppose $q\ne p$ divides $\Phi_n(a,b)$. By similar reasoning, we get that $q$ is the largest prime factor of $n$. This is a contradiction, as we established $p$ as the largest prime factor.
Hence $\Phi_n(a,b)$ is $p$. 

\textbf{Case 2 }: If $p=2$ then $n\geq3$ is a power of $2$, so $4\mid n$ implies ( as $n\ge3$). So 
...
\textbf{Case 1 }: If $\beta\geq2$ then
...
\textbf{Case 2 }: $\beta=1$, so $n=pk$. Note $p\nmid k$. Infact $k<p$.
...
\section{ Sum Version of Zsigmondy's Theorem}
...
\textbf{Walkthrough}: 
\begin{itemize}
 \item You would like to use zsigmondy here, but how?
 \item Take an arbitrary $d\mid p_1p_2\dots p_n$
 \item Can you now find distinct prime factors of $2^{p_1p_2\cdots p_n}+1$
\end{itemize}
...
\textbf{Walkthrough}: 
\begin{itemize}
 \item I will spoil it for you! Answer is yes.
 \item Start with $9$ and then inductively find the next primes using Zsigmondy's theorem.
\end{itemize}
...
\textbf{Walkthrough}:
\begin{itemize}
 \item Check the edge cases of Zsigmondy!
\end{itemize}
...
One obvious solution is $(a,n)=(1,n)$. Assume $a>1$.
  ...
  \newline
\textbf{Case 1 }: $(2,6)$ does not work by brute force checking
\newline
\textbf{Case 2 }: If $(a, n)=(2^k-1, 2)$. Then note $(2^k-1)^2-1=(2^k)(2^k-2)=(2^{k+1})(2^{k-1}-1)$. So the primes divide $2^{k-1}-1$ and $2$. So $m=1$ works as $(2^k-1)-1=2^k-2=2(2^{k-1}-1)$.
</div>
